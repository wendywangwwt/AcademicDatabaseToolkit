...
encode_keyword <- function(keyword,db,field){
  ...
  if (db %in% c('sage_journal','pubmed')){
    res <- lapply(keyword,function(x) {stringr::str_replace_all(x," ","+")}) %>% unlist() #%>%
      # paste(collapse = '+AND+')
  }else{
...
get_url <- function(keyword,field,db,api_key=NULL,subdb=NULL,start_record=NULL,
  ...
  if (db == 'sage_journal'){
    ...
    if (is.null(start_record)){
      res <- glue::glue('http://fedsearch.proquest.com/search/sru/{subdb}?operation=searchRetrieve&version=2&maximumRecords=10000&{additional_args}&query={field_name}"{query}"')
    }else{
      res <- glue::glue('http://fedsearch.proquest.com/search/sru/{subdb}?operation=searchRetrieve&version=1.2&maximumRecords=10000&{additional_args}&startRecord={start_record}&query={field_name}"{query}"')
    }
  }
